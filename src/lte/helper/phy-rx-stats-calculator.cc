...
PhyRxStatsCalculator::DlPhyReception (PhyReceptionStatParameters params)
{
  ...
  if ( m_dlRxFirstWrite == true )
    {
      dlOutFile.open (GetDlRxOutputFilename ().c_str ());
      ...
      dlOutFile << "% time\tcellId\tIMSI\tRNTI\ttxMode\tlayer\tmcs\tsize\trv\tndi\tcorrect\tccId";
      dlOutFile << "\n";
    }
  ...
  dlOutFile << (uint32_t) params.m_txMode << "\t";
  ...
  dlOutFile << (uint32_t) params.m_correctness << "\t";
...
PhyRxStatsCalculator::UlPhyReception (PhyReceptionStatParameters params)
{
  ...
  if ( m_ulRxFirstWrite == true )
    {
      ulOutFile.open (GetUlRxOutputFilename ().c_str ());
      ...
      ulOutFile << "% time\tcellId\tIMSI\tRNTI\tlayer\tmcs\tsize\trv\tndi\tcorrect\tccId";
      ulOutFile << "\n";
    }
  ...
  ulOutFile << (uint32_t) params.m_correctness << "\t";
